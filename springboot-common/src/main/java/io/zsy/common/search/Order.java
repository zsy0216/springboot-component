...
public class Order implements Serializable {
    ...
    public Order(String property, Direction direction, String orderExpr) {
        ...
        this.property = Condition.isAcronym(property);
    ...
    public String toString() {
        ...
        if (orderExpr != null && orderExpr.contains("?")) {
            ...
            if (exprs.length == 2) {
                return String.format(orderExpr.replace("\\?", "%s"), property) + (direction == null ? "" : " " + direction.name());
            }
            return String.format(orderExpr.replace("\\?", "%s"), property, direction == null ? "" : " " + direction.name());
        }
    ...
    public static List<Order> formString(String orderSegment, String orderExpr) {
        ...
        for (String sortSegment : orderSegments) {
            Order order = formString1(sortSegment, orderExpr);
            if (order != null) {
                results.add(order);
            }
        }
    ...
    private static Order formString1(String orderSegment, String orderExpr) {
        if (orderSegment == null || "".equals(orderSegment.trim()) ||
